---
description: Build plan for the supplied task.
argument-hint: [adw-id] [prompt]
---

# Plan

Create a plan to complete the task using the specified markdown `Plan Format`. 
Research the codebase and create a thorough plan appropriate to the task's complexity.
...
- IMPORTANT: Provide click-path for all configuration steps user needs to follow to complete the task, but only if it not solvable via commands.
...
- `src/` - AI platform extraction service layer you'll be working in
- `.claude/commands/` - Claude command templates
- `specs/` - Specification and plan documents per service
