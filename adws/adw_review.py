...
def run_review(
    ...
    if not response.success:
        ...
        return ReviewResult(
            success=False,
            review_summary="Review execution failed due to an error.",
            review_issues=[
                ReviewIssue(
    ...
    try:
        ...
        return ReviewResult(
            success=False,
            review_summary="Failed to parse review result from agent output.",
            review_issues=[
                ReviewIssue(
                    review_issue_number=1,
                    screenshot_path=None,
...
def main():
        ...
        if review_result.success:
            ...
            if resolved_count > 0:
                ...
                commit_msg, error = create_commit(
                    AGENT_REVIEW_PATCH_IMPLEMENTOR, review_issue, issue_command, adw_id, logger, os.getcwd()
                )
    ...
    commit_msg, error = create_commit(
        AGENT_REVIEWER, review_issue, issue_command, adw_id, logger, os.getcwd()
    )
