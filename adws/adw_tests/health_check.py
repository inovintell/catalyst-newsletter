...
Usage:
uv run adws/adw_tests/health_check.py <issue_number>
...
def check_env_vars() -> CheckResult:
    ...
    optional_vars = {
        "ANTHROPIC_API_KEY": "(Optional) For extraction service and Claude Code hooks - automatically mapped to HOOKS_ANTHROPIC_API_KEY. Claude Code CLI uses its own authentication.",
...
def run_health_check() -> HealthCheckResult:
    ...
    if os.getenv("ANTHROPIC_API_KEY"):
        ...
        result.checks["claude_code"] = CheckResult(
            success=False,
            details={"skipped": True, "reason": "ANTHROPIC_API_KEY not set (optional - Claude Code uses its own auth)"},
        )
...
def main():
    ...
    if not result.success:
        ...
        if any("ANTHROPIC_API_KEY" in e for e in result.errors):
            print("   1. Set ANTHROPIC_API_KEY in your .env file (for extraction service and hooks)")
