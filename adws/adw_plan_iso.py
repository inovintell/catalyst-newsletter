...
def main():
    ...
    # Ensure plan_file_path is relative, not absolute
    if os.path.isabs(plan_file_path):
        # Convert absolute worktree path to relative
        if plan_file_path.startswith(worktree_path):
            plan_file_path = os.path.relpath(plan_file_path, worktree_path)
            logger.info(f"Converted absolute worktree path to relative: {plan_file_path}")
        else:
            # Absolute path pointing outside worktree - ERROR!
            error = f"Plan file path must be relative to worktree root, got absolute path outside worktree: {plan_file_path}"
            logger.error(error)
            make_issue_comment(
                issue_number,
                format_issue_message(adw_id, "ops", f"❌ {error}"),
